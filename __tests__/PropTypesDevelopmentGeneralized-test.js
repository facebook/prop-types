...
function expectInvalidValidatorWarning(declaration, type) {
  expect(() => PropTypes.checkPropTypes({ foo: declaration }, { foo: {} }, 'prop', 'testComponent', null)).toThrowError(
...
describe('PropTypesDevelopmentGeneralizes', () => {
  beforeEach(() => {
    reset();
  ...
  describe('checkPropTypes', () => {
    ...
    it('should warn for identical validator failures', () => {
      const propTypes = { foo: PropTypes.number };
      const props = { foo: 'foo' };
      let errorCount = 0;
      try {
    ...
    it('rethrows error from validator', () => {
      const propTypes = {
    ...
    it('warns if any of the propTypes is not a function', () => {
      ...
      const props = { foo: 'foo' };
      expect(() => PropTypes.checkPropTypes(propTypes, props, 'prop', 'testComponent', null)).toThrowError(
        'Warning: Failed prop type: testComponent: prop type `foo` is invalid; ' +
        'it must be a function, usually from the `prop-types` package, but received `undefined`.' +
        'This often happens because of typos such as `PropTypes.function` instead of `PropTypes.func`.'
      );
  ...
  describe('OneOf Types', () => {
    it('should error for invalid argument', () => {
      expect(() => PropTypes.oneOf('red')).toThrowError(
        'Invalid argument supplied to oneOf, expected an array.',
      );
    });

    it('should error for invalid multiple arguments', () => {
      expect(() => PropTypes.oneOf('red', 'blue')).toThrowError(
  ...
  describe('Union Types', () => {
    it('should error for invalid argument', () => {
      expect(() => PropTypes.oneOfType(PropTypes.string, PropTypes.number)).toThrowError(
    ...
    it('should warn but for invalid argument type', () => {
      ...
      for (let i = 0; i < expected.length; i++) {
        const type = types[i];
        expect(() => PropTypes.oneOfType([type])).toThrowError(
